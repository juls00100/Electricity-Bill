...
// 1. Add these variables at the top of your class to store the data
String accNum, billID, amount, cash, change, date;

// 2. Update this specific constructor (the one PayBill uses)
// Hanapin ang constructor na ito sa Payment.java
public Payment(String accNum, String billID, String amount, String cash, String change, String date) {
    initComponents(); 
    this.accNum = accNum;
    this.billID = billID;
    this.amount = amount;
...
/**
    ...
    user_dashboard ud = new user_dashboard(accNum); 
    ud.setVisible(true);
    this.dispose();
    ...
    try {
        ...
        String sql = "SELECT p_id, bill_id, p_amount, p_cash, p_change, p_date "
                   + "FROM payments WHERE p_id LIKE '%" + searchTxt + "%' "
                   + "OR bill_id LIKE '%" + searchTxt + "%'";
  ...
  public void displayData() {
    try {
        config conf = new config();
        // Siguraduhin na tama ang column names base sa iyong 'payments' table
        String query = "SELECT p_id, bill_id, p_amount, p_cash, p_change, p_date FROM payments"; 
        conf.displayData(query, paymenttable); 
    } catch (Exception ex) {
        System.out.println("Error showing history: " + ex.getMessage());
    }
}
}
